...
/**
 * Aldine helper functions and namespace constants.
 *
 * The Aldine\Admin\BLOG_OPTION constant must remain a namespace constant:
 * pressbooks core checks defined('\Aldine\Admin\BLOG_OPTION').
 *
 * @package Aldine
 */
    ...
    function get_catalog_data(array $args): array
    {
        ...
        if (isset($args['sort_by_featured'])) {
            usort($books, function ($a, $b) use ($args) {
                return array_search($a['id'], $args['site__in'], true) - array_search(
                    $b['id'],
                    $args['site__in'],
                    true
                );
